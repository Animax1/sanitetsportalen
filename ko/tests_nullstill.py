...
class PorteneTests(TestCase):
    ...
    def test_fanene_paa_sida(self):
        ...
        # Tavla (22. sep. 2026): KO-lederens fane, tegnet av ko-tavle.js.
        self.assertIn('data-verdifane="tavla"', html)
        self.assertIn('koTegnTavleOppsett', html)
        self.assertNotIn('data-verdifane="nullstill"', html, 'Nullstill er global admin')
        skriver = _gi(_gi(_bruker('skriver'), 'ko', 'skriv_full'), 'oppdrag', 'skriv_leder')
        c.force_login(skriver)
        html = c.get('/ko/').content.decode()
        self.assertNotIn('data-verdifane="tavla"', html, 'oppsettet av tavla er KO-lederens')
