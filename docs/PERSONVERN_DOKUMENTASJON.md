...
| XSS-beskyttelse | Auto-escape i Djangos template-motor. I JavaScript escapes brukerdata manuelt med `escapeHtml()`/`_escHtml` i pasientskjemaet og arkivvisningen. **Statistikk-tabellene er ennå ikke dekket** — de setter feltverdier direkte i `innerHTML` (kjent avvik, se N6 i `docs/FORBEDRINGER_2026-08.md`). Risikoen dempes av at alle kliniske felt valideres mot en serverside-whitelist (`patients/choices.py`) før lagring |
