...
| `/ko/` | 21 | `ko/urls.py` | Sida, sidebaren, ansvarsmerket, loggen (les, skriv, rediger, fjern, fest, løsne), hendelsene (ny, rediger, prioritet, bli med, lag, lukk, gjenåpne, knytt oppdrag), KO-innstillingene (ansvarsområder) og nullstilling (oppdrag, hendelser, logg — global admin). Ressursene og oppdragene leses fra `/oppdrag/api/…` — se §7 i KO-notatet |
